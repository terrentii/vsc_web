...
class Message(db.Model):
    ...
    # Идемпотентность отправки из десктоп-клиента (под-проект №6). NULL допускается
    # множественно — старые/веб-сообщения без client_msg_id не конфликтуют.
...
class AuthToken(db.Model):
    """Сессионные opaque-токены входа десктоп-клиента (под-проект №6).

    Отдельно от ApiKey (долгоживущие machine-to-machine ключи бота), чтобы логаут
    инвалидировал только токен сессии и не бил по ключам.
    """
